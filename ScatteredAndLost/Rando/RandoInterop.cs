...
internal static class RandoInterop
{
    ...
    public static void Setup()
    {
        CondensedSpoilerLogger.AddCategory(
            "Bretta Hearts",
            _ => IsEnabled,
            new(BrettaHeart.All().Select(h => h.name))
        );
    ...
    private static void ExportCompleted(RandoController rc)
    {
        ...
        if (LS.RandomizeSoulTotems)
            mod.RandomizeSoulTotems = true;
        if (LS.EnableCheckpoints)
            mod.Checkpoint = LS.EnableHeartDoors
                ? CheckpointLevel.Entrance
                : CheckpointLevel.Zippers;
    }
    ...
    private static void ModifyLogic(GenerationSettings gs, LogicManagerBuilder lmb)
    {
        ...
        if (LS.EnableHeartDoors)
        {
            ...
            foreach (var h in BrettaHeart.All())
                lmb.AddItem(new StringItemTemplate(h.name, $"{BrettaHeart.TermName}++"));
        ...
        foreach (var e in RandomizerData.Transitions)
        {
            if (e.Value.Logic == null)
                continue;
            if (!LS.EnableHeartDoors && e.Value.Def!.SceneName == HEART_GATE_SCENE)
                continue;
        ...
        foreach (var e in RandomizerData.Logic)
            lmb.AddWaypoint(new(e.Key, e.Value, false));
        foreach (var e in RandomizerData.Waypoints)
            lmb.AddWaypoint(new(e.Key, e.Value, true));

        foreach (var loc in RandomizerData.Locations)
        {
            if (loc.Value.Logic == null)
                continue;
            if (!LS.EnableHeartDoors && loc.Value.Location!.sceneName == HEART_GATE_SCENE)
                continue;
    ...
    private static bool RandomizeTransition(
        TransitionDef def,
        TransitionSettings.TransitionMode mode
    )
    {
        if (!LS.EnableHeartDoors && def.SceneName == HEART_GATE_SCENE)
            return false;
    ...
    {
        if (loc.Location!.sceneName == HEART_GATE_SCENE)
            return LS.EnableHeartDoors;
        else
            return LS.RandomizeSoulTotems;
    }
    ...
    private static void ModifyRequestBuilder(RequestBuilder rb)
    {
        ...
        if (
            LS.EnableCheckpoints
            && rb.gs.TransitionSettings.Mode != TransitionSettings.TransitionMode.None
        )
            throw new System.ArgumentException(
                "Bretta House checkpoints are incompatible with transition rando"
            );
        if (LS.RandomizeSoulTotems && !rb.gs.PoolSettings.SoulTotems)
            throw new System.ArgumentException(
                "Soul Totems must be randomized if randomizing Bretta House Soul Totems"
            );
        ...
        bool matching =
            rb.gs.TransitionSettings.TransitionMatching
                == TransitionSettings.TransitionMatchingSetting.MatchingDirections
            || rb.gs.TransitionSettings.TransitionMatching
                == TransitionSettings.TransitionMatchingSetting.MatchingDirectionsAndNoDoorToDoor;
        var dualBuilder =
            rb.EnumerateTransitionGroups().FirstOrDefault(x => x.label == RBConsts.TwoWayGroup)
            as SelfDualTransitionGroupBuilder;
        var horizontalBuilder =
            rb.EnumerateTransitionGroups()
                .FirstOrDefault(x => x.label == RBConsts.InLeftOutRightGroup)
            as SymmetricTransitionGroupBuilder;
        var verticalBuilder =
            rb.EnumerateTransitionGroups().FirstOrDefault(x => x.label == RBConsts.InTopOutBotGroup)
            as SymmetricTransitionGroupBuilder;
        ...
        if (matching)
        {
            if (doors.Count > 0)
            {
                ...
                {
                    switch (
                        horizontalBuilder!.Group2.GetTotal() - horizontalBuilder.Group1.GetTotal()
                    )
                    {
                        case > 0:
                        ...
                        case 0:
                            switch (
                                verticalBuilder!.Group2.GetTotal()
                                - verticalBuilder.Group1.GetTotal()
                            )
                            {
                                case > 0:
        ...
        if (LS.EnableHeartDoors)
        {
            if (!rb.gs.PoolSettings.Keys)
                throw new System.ArgumentException(
                    "Cannot enable heart doors without randomizing keys"
                );
            ...
            for (int i = 0; i < numHearts; i++)
                rb.AddItemByName(allHearts[r.Next(allHearts.Count)].name);
        }
        else
        {
            ...
            rb.EditTransitionRequest(
                ...
                info =>
                {
                    info.AddGetTransitionDefModifier(
                        BrettaDoorIn,
                        def =>
                            def with
                            {
                                VanillaTarget = "BrettaHouseZippers[right1]",
        ...
        foreach (var loc in RandomizerData.Locations)
        {
            ...
            if (LS.RandomizeSoulTotems && loc.Value.Location is SuperSoulTotemLocation)
                rb.AddItemByName(SuperSoulTotemItem.NAME);
        }
    }

    private const string SoulTotemsTerm = "BRETTAHOUSESOULTOTEMS";

    private static void OnCreateProgressionInitializer(
        LogicManager lm,
        GenerationSettings gs,
        ProgressionInitializer prog
    )
    {
        if (!LS.Enabled || !LS.EnableHeartDoors)
            return;
