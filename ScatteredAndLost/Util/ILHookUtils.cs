...
internal static class ILHookUtils
{
    ...
    public static void GotoRange(
        this ILCursor self,
        Func<Instruction, bool> first,
        Func<Instruction, bool> last
    )
    {
        self.GotoNext(last);
        self.GotoPrev(first);
    ...
    public static bool MatchLdfldName(this Instruction instr, string name) =>
        instr.MatchLdfld(out var info) && info.Name == name;
}
