...
internal static class DecorationMasterUtil
{
    internal static void RegisterDecoration<B, I>(string name, GameObject prefab, string spriteName)
    {
        ...
        using Stream stream = typeof(ScatteredAndLostMod).Assembly.GetManifestResourceStream(
            $"HK8YPlando.Resources.Sprites.{spriteName}.png"
        );
    ...
    internal static void RefreshItemManager()
    {
        ...
        group.Clear();
        int idx = 0;
        for (int i = 0; i < items.Count; i += ItemManager.GroupMax)
            group.Add(
                ++idx,
                [.. items.GetRange(i, Math.Min(ItemManager.GroupMax, items.Count - i))]
            );
        ItemManager.Instance.SwitchGroup(0);
    }

    private static List<string> GetDecoMasterPairs() =>
        [
            .. ObjectLoader
                .InstantiableObjects.Where(x =>
                {
                    ...
                    if (settings.ProfessorMode)
                        return !cd.GetType().IsDefined(typeof(ObsoleteAttribute), false);
                    else if (settings.MemeItem)
                        return !cd.GetType().IsDefined(typeof(AdvanceDecoration), false);
                    else
                        return !cd.GetType().IsDefined(typeof(AdvanceDecoration), false)
