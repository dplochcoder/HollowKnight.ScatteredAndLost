...
internal class Zipper : MonoBehaviour
{
    ...
    [ShimField]
    public List<AudioClip> TouchClips = [];

    [ShimField]
    public List<AudioClip> ImpactClips = [];

    [ShimField]
    public AudioClip? RewindIntro;

    [ShimField]
    public AudioClip? RewindLoop;

    [ShimField]
    public List<AudioClip> ResetClips = [];

    [ShimField]
    public ZipperPlatform? Platform;

    [ShimField]
    public GameObject? BottomHurtBox;
    ...
    private void Awake()
    {
        ...
        [
            .. gameObject
                .FindComponentsRecursive<ZipperLineCog>()
                .Select(b => b.gameObject.GetComponent<SpriteRenderer>()),
        ];
        platCogs =
        [
            .. gameObject.FindComponentsRecursive<ZipperPlatformCog>().Select(b => b.gameObject),
        ];
    ...
    private IEnumerator<CoroutineElement> Run()
    {
        ...
        while (true)
        {
            ...
            var shootTime =
                (Mathf.Sqrt(2 * Accel * travelDist + StartSpeed * StartSpeed) - StartSpeed) / Accel;
            bool disableBottomSpikes =
                BottomHurtBox!.activeSelf && (targetPos.y - restPos.y) >= -0.1f;
            ...
            yield return Coroutines.SleepSecondsUpdateDelta(
                ...
                _ =>
                {
                    Platform!.SpriteShaker!.transform.localPosition = MathExt.RandomInCircle(
                        Vector2.zero,
                        ShakeRadius
                    );
                    return false;
            ...
            if (disableBottomSpikes)
                BottomHurtBox!.SetActive(false);
            ...
            yield return Coroutines.SleepSecondsUpdateDelta(
                ...
                time =>
                {
                    ...
                    var d =
                        StartSpeed * launchTime.Value
                        + Accel * launchTime.Value * launchTime.Value / 2;
            ...
            yield return Coroutines.SleepSecondsUpdatePercent(
                ...
                {
                    if (!audio.isPlaying)
                        Platform.gameObject.LoopSound(RewindLoop!, 1f, false);
...
{
    [ShimField]
    public GameObject? SpriteShaker;
...
internal class ZipperDecoration : CustomDecoration
{
    public static void Register() =>
        DecorationMasterUtil.RegisterDecoration<ZipperDecoration, ZipperDecorationItem>(
    ...
    private void Start()
    {
        ...
        ZipperLib.UpdateZipperAssets(
            gameObject,
            itemTyped.TopSpikes == 1,
            itemTyped.RightSpikes == 1,
            itemTyped.BotSpikes == 1,
            itemTyped.LeftSpikes == 1,
            _ => { }
        );
    }
    ...
    public void SetXMove(float x) =>
        gameObject.GetComponent<Zipper>().UpdateTargetPos(p => p with { x = x });

    [Handle(Operation.SetSizeY)]
    public void SetYMove(float y) =>
        gameObject.GetComponent<Zipper>().UpdateTargetPos(p => p with { y = y });

    [Handle(Operation.SetColorA)]
    public void SetTopSpikes(int value) =>
        gameObject.GetComponent<Zipper>().SetTopSpikes(value == 1);

    [Handle(Operation.SetColorR)]
    public void SetRightSpikes(int value) =>
        gameObject.GetComponent<Zipper>().SetRightSpikes(value == 1);

    [Handle(Operation.SetColorG)]
    public void SetBotSpikes(int value) =>
        gameObject.GetComponent<Zipper>().SetBotSpikes(value == 1);

    [Handle(Operation.SetColorB)]
    public void SetLeftSpikes(int value) =>
        gameObject.GetComponent<Zipper>().SetLeftSpikes(value == 1);
}
