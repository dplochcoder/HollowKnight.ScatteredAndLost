...
internal class Bumper : MonoBehaviour, IHitResponder
{
    ...
    [ShimField]
    public float HorizontalBumpX;

    [ShimField]
    public float HorizontalBumpY;

    [ShimField]
    public float HorizontalBumpYMax;
    ...
    public void Hit(HitInstance damageInstance)
    {
        ...
        if (damageInstance.AttackType != AttackTypes.Nail)
            return;
        if (KnightUtil.IsNailArtActive())
            return;

        var cState = HeroController.instance.cState;
        if (cState.downAttacking)
            BumperSpeedControl.BumpUp(VerticalScale);
        else if (cState.upAttacking)
            BumperSpeedControl.BumpDown();
        else
        {
            var kPos = HeroController.instance.gameObject.transform.position;
            BumperSpeedControl.Instance.BumpHorizontal(
                HorizontalBumpX,
                HorizontalDecel,
                (transform.position.y - kPos.y > YForgiveness) ? 0 : HorizontalBumpY,
                HorizontalBumpYMax
            );
        }
    }
    ...
    private void Update()
    {
        ...
        if (oscillateTimer > OscillatePeriod)
            oscillateTimer -= OscillatePeriod;

        Vector3 delta = new(
            OscillateRadius * Mathf.Sin(2 * oscillateTimer * Mathf.PI / OscillatePeriod),
            0,
            0
        );
...
public class BumperDecoration : CustomDecoration
{
    public static void Register() =>
        DecorationMasterUtil.RegisterDecoration<BumperDecoration, BumperDecorationItem>(
    ...
    public override void HandlePos(Vector2 val) =>
        gameObject.GetComponent<Bumper>().SetOrigPos(val);
}
