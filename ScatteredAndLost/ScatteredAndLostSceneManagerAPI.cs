...
public static class ScatteredAndLostSceneManagerAPI
{
    ...
    private static AssetBundle LoadCoreBundle()
    {
        ...
        try
        {
            ...
            var debugData =
                PurenailCore.SystemUtil.JsonUtil<ScatteredAndLostMod>.DeserializeEmbedded<Data.DebugData>(
                    "HK8YPlando.Resources.Data.debug.json"
                );
            var bundle = AssetBundle.LoadFromFile(
                $"{debugData.LocalAssetBundlesPath}/{CORE_BUNDLE_NAME}"
            );
        ...
        {
            ScatteredAndLostMod.BUG($"Failed to load {CORE_BUNDLE_NAME} from local assets: {e}");
        }
#endif

        using StreamReader sr = new(
            typeof(ScatteredAndLostSceneManagerAPI).Assembly.GetManifestResourceStream(CORE_BUNDLE)
        );
